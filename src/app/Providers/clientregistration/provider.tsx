...
export const ClientRegistrationProvider = ({ children }: { children: React.ReactNode }) => {
  const [state, dispatch] = useReducer(ClientRegistrationReducer, INITIAL_STATE);
  
  const registerClient = async (clientData: IClientRegistration) => {
    dispatch(registerClientPending());
    
    try {
      const response = await axiosInstance.post('/api/users/register/mobile', clientData);
      ...
      console.error(error);
      const errorMessage = error.response?.data?.message || "Failed to register client";
      dispatch(registerClientError(errorMessage));
    }
  };
...
export const useClientRegistrationState = () => {
  ...
  if (!context) {
    throw new Error("useClientRegistrationState must be used within a ClientRegistrationProvider");
  }
  return context;
};
...
export const useClientRegistrationActions = () => {
  ...
  if (!context) {
    throw new Error("useClientRegistrationActions must be used within a ClientRegistrationProvider");
  }
  return context;
};